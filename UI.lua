...
function GC:CreateProposePanel()
    ...
    local updateBtn = CreateFrame("Button", nil, panel)
    updateBtn:SetSize(140, 22)
    updateBtn:SetPoint("TOPLEFT", panel, "TOPLEFT", 20, -118)
    local updateBg = updateBtn:CreateTexture(nil, "BACKGROUND")
    updateBg:SetAllPoints()
    updateBg:SetTexture("Interface\\Tooltips\\UI-Tooltip-Background")
    updateBg:SetVertexColor(0.15, 0.25, 0.4, 0.9)
    local updateLbl = updateBtn:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    updateLbl:SetAllPoints()
    updateLbl:SetJustifyH("CENTER")
    updateLbl:SetText(L["UI_UpdateBroadcast"] or "Update broadcast")
    updateBtn:SetScript("OnClick", function()
        GC:SendMyData()
    end)
    updateBtn:SetScript("OnEnter", function()
        updateBg:SetVertexColor(0.2, 0.35, 0.55, 1)
    end)
